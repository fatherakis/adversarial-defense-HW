...
void run_CNN(vart::Runner* runner) {
  ...
  for (unsigned int n = 0; n < images.size(); n += batchSize) {
    ...
    {
      Mat image = imread(baseImagePath + images[n + i], IMREAD_GRAYSCALE);

      /*image pre-process*/
      Mat image2; //  = cv::Mat(inHeight, inWidth, CV_8UC1); //CV_8SC3 is for 3 channels
      resize(image, image2, Size(inHeight, inWidth), 0, 0, INTER_NEAREST);
      for (int h = 0; h < inHeight; h++) {
        for (int w = 0; w < inWidth; w++) {
            //imageInputs[i*inSize+h*inWidth+w] = (int8_t)( ( (image2.at<unsigned char>(h, w)[c] / 255.0f  - mean[c] ) /std_dev[c] ) *2 *input_scale ); //in BGR mode
	    imageInputs[i*inSize+ h*inWidth+ w ] = static_cast<int8_t>( (image2.at<uchar>(h, w)/255.0f)*input_scale ); //in RGB mode
