    ...
    for (unsigned int i = 0; i < runSize; i++)
    {
      ...
      for (int h = 0; h < inHeight; h++) {
        for (int w = 0; w < inWidth; w++) {
          for (int c = 0; c < 3; c++) {
            //imageInputs[i*inSize+h*inWidth*3+w*3 +  c] = (int8_t)( ( (image2.at<Vec3b>(h, w)[c] / 255.0f  - mean[c] ) /std_dev[c] ) *2 *input_scale ); //in BGR mode
	    imageInputs[i*inSize+h*inWidth*3+w*3 +2-c] = (int8_t)( ( (image2.at<Vec3b>(h, w)[c]/255.0f - mean[2-c] ) /std_dev[2-c] ) *input_scale ); //in RGB mode
          }
        }
      }
